...
typedef NS_ENUM(NSUInteger, MMNumberKeyboardButton) {
  ...
  MMNumberKeyboardButtonNumberMax = MMNumberKeyboardButtonNumberMin +
                                    10, // Ten digits.
...
+ (_MMNumberKeyboardButton *)keyboardButtonWithStyle:
    (MMNumberKeyboardButtonStyle)style;
...
- (void)addTarget:(id)target
                                action:(SEL)action
    forContinuousPressWithTimeInterval:(NSTimeInterval)timeInterval;
...
+ (id)MM_currentFirstResponder {
  ...
  [[UIApplication sharedApplication]
      sendAction:@selector(MM_findFirstResponder:)
              to:nil
            from:nil
        forEvent:nil];
...
#define UIKitLocalizedString(key)                                              \
...
- (void)_commonInit {
  ...
  for (MMNumberKeyboardButton key = numberMin; key < numberMax; key++) {
    UIButton *button = [_MMNumberKeyboardButton
  ...
  UIImage *backspaceImage =
      [self.class _keyboardImageNamed:@"MMNumberKeyboardDeleteKey.png"];
  UIImage *dismissImage =
      [self.class _keyboardImageNamed:@"MMNumberKeyboardDismissKey.png"];

  UIButton *backspaceButton = [_MMNumberKeyboardButton
      keyboardButtonWithStyle:MMNumberKeyboardButtonStyleGray];
  [backspaceButton
      setImage:[backspaceImage
                   imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate]
      forState:UIControlStateNormal];

  [(_MMNumberKeyboardButton *)backspaceButton
                               addTarget:self
                                  action:@selector(_backspaceRepeat:)
      forContinuousPressWithTimeInterval:0.15f];

  [buttonDictionary setObject:backspaceButton
                       forKey:@(MMNumberKeyboardButtonBackspace)];

  UIButton *specialButton = [_MMNumberKeyboardButton
      keyboardButtonWithStyle:MMNumberKeyboardButtonStyleGray];

  [buttonDictionary setObject:specialButton
                       forKey:@(MMNumberKeyboardButtonSpecial)];

  UIButton *doneButton = [_MMNumberKeyboardButton
      keyboardButtonWithStyle:MMNumberKeyboardButtonStyleDone];
  [doneButton.titleLabel setFont:doneButtonFont];
  [doneButton setTitle:UIKitLocalizedString(@"Done")
              forState:UIControlStateNormal];
  ...
  [decimalPointButton setTitle:decimalSeparator ?: @"."
                      forState:UIControlStateNormal];
  ...
  for (UIButton *button in buttonDictionary.objectEnumerator) {
    ...
    [button addTarget:self
                  action:@selector(_buttonInput:)
        forControlEvents:UIControlEventTouchUpInside];
    [button addTarget:self
                  action:@selector(_buttonPlayClick:)
  ...
  UIPanGestureRecognizer *highlightGestureRecognizer =
      [[UIPanGestureRecognizer alloc]
          initWithTarget:self
                  action:@selector(_handleHighlightGestureRecognizer:)];
  ...
  // Add default action.
  [self configureSpecialKeyWithImage:dismissImage
                              target:self
...
- (void)_handleHighlightGestureRecognizer:
  ...
  if (gestureRecognizer.state == UIGestureRecognizerStateChanged ||
    ...
    for (UIButton *button in self.buttonDictionary.objectEnumerator) {
      BOOL points =
          CGRectContainsPoint(button.frame, point) && !button.isHidden;
...
- (void)_buttonInput:(UIButton *)button {
  ...
  if (keyboardButton >= numberMin && keyboardButton < numberMax) {
    ...
    if ([delegate
            respondsToSelector:@selector(numberKeyboard:shouldInsertText:)]) {
      BOOL shouldInsert =
          [delegate numberKeyboard:self shouldInsertText:string];
      if (!shouldInsert) {
        return;
      }
    }
  ...
  // Handle .
    ...
    if ([delegate
            respondsToSelector:@selector(numberKeyboard:shouldInsertText:)]) {
      BOOL shouldInsert =
          [delegate numberKeyboard:self shouldInsertText:decimalText];
      if (!shouldInsert) {
        return;
      }
    }
...
- (void)configureSpecialKeyWithImage:(UIImage *)image
  ...
  [self configureSpecialKeyWithImage:image
                       actionHandler:^{
                         __strong __typeof(&*weakTarget) strongTarget =
                             weakTarget;
                         __strong __typeof(&*weakSelf) strongSelf = weakSelf;

                         if (strongTarget) {
                           NSMethodSignature *methodSignature =
                               [strongTarget methodSignatureForSelector:action];
...
- (void)setReturnKeyTitle:(NSString *)title {
  if (![title isEqualToString:self.returnKeyTitle]) {
    ...
    if (button) {
      NSString *returnKeyTitle = (title != nil && title.length > 0)
                                     ? title
                                     : [self defaultReturnKeyTitle];
...
- (void)setReturnKeyButtonStyle:(MMNumberKeyboardButtonStyle)style {
  if (style != _returnKeyButtonStyle) {
    ...
    _MMNumberKeyboardButton *button =
        self.buttonDictionary[@(MMNumberKeyboardButtonDone)];
    if (button) {
      button.style = style;
    }
  }
...
- (void)layoutSubviews {
  ...
  const CGFloat spacing = (interfaceIdiom == UIUserInterfaceIdiomPad)
                              ? MMNumberKeyboardPadBorder
                              : 0.0f;
  const CGFloat maximumWidth = (interfaceIdiom == UIUserInterfaceIdiomPad)
                                   ? 400.0f
  ...
  const CGRect contentRect =
      (CGRect){.origin.x = MMRound((CGRectGetWidth(bounds) - width) / 2.0f),
               .origin.y = spacing,
               .size.width = width,
  ...
  for (MMNumberKeyboardButton key = numberMin; key < numberMax; key++) {
    ...
    if (digit == 0) {
      ...
      if (!allowsDecimalPoint) {
        ...
        [button
            setContentEdgeInsets:UIEdgeInsetsMake(0, 0, 0, numberSize.width)];
      }
  ...
  if (interfaceIdiom != UIUserInterfaceIdiomPad) {
    ...
    if (separatorViews.count != numberOfSeparators) {
      ...
      if (removes) {
        NSIndexSet *indexes =
            [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, delta)];
        [[separatorViews objectsAtIndexes:indexes]
    ...
    const CGFloat separatorDimension =
        1.0f / (self.window.screen.scale ?: 1.0f);

    [separatorViews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx,
                                                 BOOL *stop) {
...
- (CGSize)sizeThatFits:(CGSize)size {
  ...
  size.height =
      MMNumberKeyboardRowHeight * MMNumberKeyboardRows + (spacing * 2.0f);
  ...
  if (resource) {
    ...
    if (bundle) {
      NSString *resourcePath =
          [bundle pathForResource:resource ofType:extension];
...
- (void)_buttonStyleDidChange {
  ...
  if (style == MMNumberKeyboardButtonStyleWhite) {
    ...
    highlightedFillColor =
        [UIColor colorWithRed:0.82f green:0.837f blue:0.863f alpha:1];
  ...
  [self setTitleColor:highlightedControlColor
  ...
  if (interfaceIdiom == UIUserInterfaceIdiomPad) {
    ...
    buttonLayer.shadowColor =
        [UIColor colorWithRed:0.533f green:0.541f blue:0.556f alpha:1].CGColor;
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
  ...
  const NSTimeInterval continuousPressTimeInterval =
      self.continuousPressTimeInterval;
...
- (void)_beginContinuousPress {
  ...
  self.continuousPressTimer = [NSTimer
      scheduledTimerWithTimeInterval:continuousPressTimeInterval
                              target:self
...
- (void)_beginContinuousPressDelayed {
  [self performSelector:@selector(_beginContinuousPress)
             withObject:nil
             afterDelay:self.continuousPressTimeInterval * 2.0f];
}

- (void)_cancelContinousPressIfNeeded {
  [NSObject
      cancelPreviousPerformRequestsWithTarget:self
                                     selector:@selector(_beginContinuousPress)
                                       object:nil];
